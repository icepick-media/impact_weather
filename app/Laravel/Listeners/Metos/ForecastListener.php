    ...
    public function handle(Forecast $data){
        if(count($data->forecast) > 1){
            ...
            foreach($data->forecast as $index => $forecast){
                ...
                $is_exist = MetosForecast::where('station_id',$station_id)
                            ->where('hour',Helper::date_format($forecast['0'],"g"))
                            ->whereRaw("DATE(schedule) = '{$date}'")->first();
                if(!$is_exist){
                    $rec = [
                        'station_id' => $station_id,
                        'schedule' => Helper::datetime_db($forecast['0']),
                        'hour'  => Helper::date_format($forecast['0'],"g"),
                        ];
                    ...
                    foreach($fields as $j => $field){
                       $current_index = $j+1;
                       $appends[$field] = trim(isset($forecast[$current_index]) ? $forecast[$current_index] : "0");
                    }

                    array_push($for_insert, $rec+$appends);
                }
            }

            MetosForecast::insert($for_insert);
            // foreach($data->forecast as $index => $forecast){
            //     if($index ==0){ continue; }
            //     $date = Helper::date_db($forecast['0']);
